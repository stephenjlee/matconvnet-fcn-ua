function fcnTrain_satellite(varargin)
...
opts.expDir = 'data/fcn8s-satellite' ;
opts.dataDir = 'data/satellite' ;
...
opts.vocEdition = '11' ;
opts.vocAdditionalSegmentations = false ; %turn to false
...
opts.train.learningRate = 0.0001 * ones(1,200) ;
...
if exist(opts.imdbPath)
  ...
  imdb = vocSetup_satellite('dataDir', opts.dataDir, ...
...
if exist(opts.imdbStatsPath)
  ...
  stats = getDatasetStatistics_satellite(imdb) ;
...
if strcmp(opts.modelType, 'fcn8s')
  ...
  net = fcnInitializeModel8s_satellite(net) ;
end
...
% -------------------------------------------------------------------------
fn = @(imdb,batch) getBatch_satellite(imdb,batch,opts,'prefetch',nargout==0) ;
